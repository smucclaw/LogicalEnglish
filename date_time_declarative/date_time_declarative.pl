:- module(date_time_declarative, [
  is_valid_date/1, is_valid_date_z3/1,
  date_get/2,
...
is_duration_before_dates(Date0, Duration, Date1) :-
  ...
  ( maplist(integer, [Duration_num, Day0, Month0, Year0]), !,
    ...
    % is_valid_date_pair_z3(Date0, Date1),
    % writeln([Date0, Date1]),
...
is_valid_date_z3(date(Year, Month, Day)) :-
...
is_valid_date_pair_z3(date(Year0, Month0, Day0), date(Year1, Month1, Day1)) :-
